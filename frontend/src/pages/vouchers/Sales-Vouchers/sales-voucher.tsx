...
const SalesVoucherPage: React.FC = () => {
  ...
  const { data: voucherList, isLoading: isLoadingList } = useQuery(
    ['salesVouchers'],
    () => voucherService.getVouchers('sales_voucher')
  );
  ...
  const { data: voucherData, isLoading: isFetching } = useQuery(
    ...
    () => voucherService.getVoucherById('sales_voucher', selectedId!),
    { enabled: !!selectedId }
  );
  ...
  const createMutation = useMutation((data: any) => voucherService.createVoucher('sales_voucher', data), {
    onSuccess: () => {
  ...
  const updateMutation = useMutation((data: any) => voucherService.updateVoucher('sales_voucher', selectedId!, data), {
    onSuccess: () => {
      queryClient.invalidateQueries('salesVouchers');
      setMode('view');
    },
