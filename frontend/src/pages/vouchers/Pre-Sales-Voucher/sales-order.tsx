...
import { getCustomers, getProducts } from '../../../services/masterService';  // Assuming for sales-order (similar to sales)
...
const SalesOrderPage: React.FC = () => {
  ...
  const { control, handleSubmit, reset, setValue, watch, formState: { errors } } = useForm({
    defaultValues: {
      voucher_number: 'SO/2526/00000001',  // Adjusted for sales-order
  ...
  const { data: voucherList, isLoading: isLoadingList } = useQuery(
    ['salesOrders'],
    () => voucherService.getVouchers('sales_order')  // Adjusted to getVouchers with type 'sales_order'
  );
  ...
  const { data: voucherData, isLoading: isFetching } = useQuery(
    ...
    () => voucherService.getVoucherById('sales_order', selectedId!),
    { enabled: !!selectedId }
  );
  ...
  const createMutation = useMutation((data: any) => voucherService.createVoucher('sales_order', data), {
    onSuccess: () => {
  ...
  const updateMutation = useMutation((data: any) => voucherService.updateVoucher('sales_order', selectedId!, data), {
    onSuccess: () => {
      queryClient.invalidateQueries('salesOrders');
      setMode('view');
    },
